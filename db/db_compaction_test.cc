...
TEST_P(DBCompactionTestWithParam,
  ...
  // ingest some files for IntraL0Compaction that overlap with the memtable
  for (int i = 4; i < 6; i++) {
    ASSERT_EQ(6 + (i - 4), NumTableFilesAtLevel(0));
    IngestOneKeyValue(dbfull(), Key(i), value, options);
  }
  ...
  ASSERT_EQ(8, NumTableFilesAtLevel(0));
  // ingest rest of files for IntraL0Compaction that do not overlap with the memtable
  for (int i = 6; i < 8; i++) {
    ASSERT_EQ(8 + (i - 6), NumTableFilesAtLevel(0));
    IngestOneKeyValue(dbfull(), Key(i), value, options);
  }
  ASSERT_EQ(10, NumTableFilesAtLevel(0));
  ...
  for (int i = 4; i < 6; i++) {
    // Make sure the ingested files didn't overwrite the keys in memtable via an
    // intra-L0 compaction.
    ASSERT_EQ(value + value, Get(Key(i)));
  }
}
